...
var Connection = function(parent, socket, main, mode, address) {
  var self = this;
  self.parent = parent;
  self.mode = mode;
...
Connection.prototype.handle = function() {
  ...
  var empty = new Buffer(8);
  empty.fill(0);
  var clients = [];
  self.main.connections.forEach(function(connection,id){
    if (!Payload.compareBuffer8(connection.identity.id, empty)) {
      clients.push({
        name: connection.identity.name,
        id: connection.identity.id.toJSON()
      });
    }
  });
  self.send(Payload.create(self.main.identity.id, idGenerator.next(), 'node::hello', JSON.stringify({ identity: self.main.identity, connections: clients })));
      ...
      switch (payload.event) {
        case "node::hello":
          ...
          if (isAlreadyConnected) {
            ...
            self.connections = [];
            _.forEach(json.connections, function(connection){
              self.connections.push({
                name: connection.name,
                id: new Buffer(connection.id)
              });
            });
