...
var Export = {
	...
	create: function(b_owner_id, owner_name, b_msg_id, event, data) {
		...
		if (data_str.length > Math.pow(2,16)-1) {
			throw new Error('Fatal Error: data string is too long (> 65k chars)');
		}
		/** Source ID */
	...
	parse: function(payload) {
		...
		var data = payload.toString('utf8', i, i+data_len); i+=data_len;
		if (i!==payload.length) {
			throw new Error('Fatal Error: payload size mismatch');
		}
