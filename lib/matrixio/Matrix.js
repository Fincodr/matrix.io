    ...
    if (options.endpoints) {
      ...
      _.forEach(networkInterfaces, function(networkInterface){
        _.forEach(networkInterface, function(networkEndpoint){
          if (networkEndpoint.family === 'IPv4') {
            // get free port for this network
            portfinder.getPort({
              port: 33300
            }, function(err,port) {
              if (!err) {                
                var server = new Server('http://' + networkEndpoint.address + ':' + port, self);
                self.servers.push(server);
                server.listen();
              }
            });
        ...
        if (names.length === 2 && names[0]==='matrix') {
          ...
          _.forEach(self.clients, function(client){
            if (client.endpoint.address===names[1]) {
              if (client.state === client.STATES.SERVICE_DOWN) {
                client.state = client.STATES.NOT_CONNECTED;
              }
              found = true;
              return false;
            }
          ...
          if (!found) {
            ...
            self.clients.push(new Client(names[1], self));
          }
        }
      });
        ...
        if (names.length === 2 && names[0]==='matrix') {
          ...
          _.forEach(self.clients, function(client){
            if (client.endpoint.address===names[1]) {
              ...
              client.disconnect();
              client.state = client.STATES.SERVICE_DOWN;
              found = true;
              return false;
            }
          });
              ...
              switch (line) {
                ...
                case '/nodes':
                  self.nodes.forEach(function(node, id){
                    console.log((' ' + id + ' ').red.inverse, node.state.name.blue, node.type.cyan.inverse, node.endpoint.address, node.endpoint.identity?node.endpoint.identity:'(no identity)');
                    /*
                ...
                case '/endpoints':
                  _.forEach(self.servers, function(server){
                    console.log((' ' + server.endpoint.address + ' ').red.inverse, server.state.name.blue);
                  });
                  break;
                case '/clients':
                  _.forEach(self.clients, function(client){
                    console.log((' ' + client.endpoint.address + ' ').red.inverse, client.state.name.blue, client.endpoint.address);
                  });
                  break;
              }
            } else {
...
Matrix.prototype.createNode = function(parent) {
  var self = this;
  if (!self.nodes.has(parent.endpoint.address)) {
    // add new node and return it
    var node = new Node(parent, self);
    self.nodes.set(parent.endpoint.address, node);
    return node;
  } else {
    // return null
    return null;
...
Matrix.prototype.removeNode = function(parent) {
  var self = this;
  if (self.nodes.has(parent.endpoint.address)) {
    var node = self.nodes.get(parent.endpoint.address);
    node.destroy();
    self.nodes.remove(parent.endpoint.address);
    node = null;
    return true;
  } else {
    return false;
...
Matrix.prototype.getNode = function(parent) {
  var self = this;
  if (self.nodes.has(parent.endpoint.address)) {
    return self.nodes.get(parent.endpoint.address);
  } else {
    return null;
  }
}

Matrix.prototype.getNodeByAddress = function(address) {
  var self = this;
  if (self.nodes.has(address)) {
