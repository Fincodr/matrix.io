...
var Matrix = function(options) {
  ...
  try {
    ...
    self.connections = new Connections(self);
    ...
    if (self.options.mdns && self.options.discover) {
          ...
          _.forEach(self.clients, function(client){
            if (client.endpoint.address===names[1]) {
              if (client.state === client.STATES.CLIENT_INACTIVE) {
                ...
                console.log('+ ', names[1], '(previously inactive)');
              }
              found = true;
              return false;
            }
      ...
      browser.on('serviceDown', function(service) {
        ...
        if (names.length === 2 && names[0]==='matrix') {
          ...
          if (!found) {
            _.forEach(self.clients, function(client){
              if (client.endpoint.dynamic && client.endpoint.address===names[1] && client.state !== client.STATES.CLIENT_INACTIVE) {
                console.log('- ', names[1]);
                client.disconnect();
...
Matrix.prototype.shutdown = function() {
  var self = this;
